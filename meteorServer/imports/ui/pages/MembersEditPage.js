...
const MemberEditPage = React.createClass({
  ...
  getVoucher() {
    this.props.vouchers.map(voucher => ({ value: voucher._id, lable: voucher.value }),
        );
  },
  renderVouchers() {
    const filteredVouchers = this.props.vouchers;
    return filteredVouchers.map((voucher) => {
      let quantityMap = {};
      for (const key in this.props.member.vouchers) {
        if (key == voucher._id) {
      ...
      return (
        <AddVoucher
key={voucher._id} customer={this.props.member.customer} quantityMap={quantityMap}
                            voucher={voucher} />
      );
    });
  },
  ...
  renderEstamps() {
    const filteredEstamps = this.props.estamps;
    return filteredEstamps.map((estamp) => {
      let active = false;
      for (const key in this.props.member.estamps) {
        if (key == estamp._id) {
